...
We next the basic ILP problem, outlined in Eq.~\eqref{eq:max}--Eq.~\eqref{eq:subtour} to personalized constraints.
...
To impose these ordering constraints, we use the \emph{start time} for each POI, denoted by $s_i$ for node $v_i$.
...
Eq.~\eqref{eq:ybound1} ensures that if $f_i \geq 0.5$, then $y_i = 1$, \ari{if $f_i \geq 0.5$, then $y_i \ge \epsilon$.} i.e., the POI must be chosen.  Eq.~\eqref{eq:ybound2} ensures that if $f_i
< 0.5$, then $y_i = 0$, \ari{if $f_i < 0.5$, then $y_i < 1 + \epsilon$.} i.e., the POI must not be chosen.  ($\epsilon$ is a
