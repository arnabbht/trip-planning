...
\begin{itemize}
    \item \textbf{RQ1:} How does the proposed work compare against the existing works in terms of itinerary personalization?

    \item \textbf{RQ2:} How does the utility yield of the TRIP solution compare against the existing baseline?

    \item \textbf{RQ3:} How does the TRIP solution perform across different city geographies?

    \item \textbf{RQ4:} How does the multimodal transportation capability offered by the TRIP solution improve the user experience?

    \item \textbf{RQ5:} How do the itineraries proposed by the TRIP solution utilize the time and cost budget?

    \item \textbf{RQ6:} How does a multi-day itinerary covering $k$ days differ from $k$ disjoint single-day itineraries?

    \item \textbf{RQ7:} What is the impact of personalized constraints (namely, number of must see POIs, number of must avoid POIs, Number of ordering constraints and number of category constraints) and different utility variants (namely, binary, continuous and hybrid) on the user experience?

    \item \textbf{RQ8:} How does dynamic replanning adapt to unexpected delays or changes in the itinerary?

    \item \textbf{RQ9:} How scalable is the TRIP solution in terms of the number of POIs and the number of days in the itinerary?
\end{itemize}
...
To evaluate the performance of the \trip solution, we identified the works \citep{bolzoni2014efficient,taylor2018tour} and \citep{chen2014automatic,vanzelst2016itinerary} as potential baselines for single day itinerary planning and multi-day itinerary planning, respectively. This choice was based on the features of the solutions, as listed in Table~\ref{tab:otherworks}. Although we requested the authors of the respective works to share their implementation codes we did not receive a suitable response. Moreover, while the works \citep{bolzoni2014efficient,taylor2018tour}  consider utility optimization as their objective, the works \citep{chen2014automatic,vanzelst2016itinerary} did not. In addition, the implementation details stated in \citep{bolzoni2014efficient} were insufficient to reproduce their solution.  We, thus, did a best-effort implementation of~\citep{taylor2018tour}. While it too lacked a code repository or implementation details like the POI visiting durations, we were able to at least replicate the key features and constraints described. 
...
system and~\citep{taylor2018tour}.
This pertains to the POI visit time field.
Since the code and dataset of~\citep{taylor2018tour} is not available, we are not sure what values they used for POI visit times.
Nevertheless, we assume that they will not be very different from the average values used by us.
Another limitation of the algorithm of~\citep{taylor2018tour} was that it did not consider any cost budget during
itinerary planning.
...
\subsection{Comparision of Multiday Itinerary planner vs K single-day itinerary planner}
...
\subsection{Performance evaluation of Personalized Constraints}
